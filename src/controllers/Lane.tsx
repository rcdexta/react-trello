...
export const Lane: FC<PropsWithChildren<LaneProps>> = ({
  ...
  const addNewCard = (params: {title?: string; laneId?: string; description?: string; label?: string}) => {
  ...
  const onDragStart = ({payload}) => {
    handleDragStart && handleDragStart(payload.id, id)
  }
  const onDragEnd = (laneId: string, result: {addedIndex: any; payload: any}) => {
    ...
    if (addedIndex != null) {
      ...
      handleDragEnd && handleDragEnd(payload.id, laneId, id, addedIndex, newCard)
  ...
  const renderDragContainer = (isDraggingOver: boolean) => {
    ...
    return (
      <components.ScrollableLane ref={laneDidMount} isDraggingOver={isDraggingOver}>
        <Container
        ...
        {editable && !addCardMode && <components.AddCardLink onClick={() => setAddCardMode(true)} t={t} />}
        {addCardMode && (
          <components.NewCardForm onCancel={() => setAddCardMode(false)} t={t} laneId={id} onAdd={addNewCard} />
        )}
